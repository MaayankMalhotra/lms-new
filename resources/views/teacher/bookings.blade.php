                    ...
                    <table class="min-w-full text-sm text-left text-gray-600">
                        ...
                        <tbody>
                            @foreach($slots as $slot)
                                @php $bookings = $slot->bookings; @endphp
                                @if($bookings->isEmpty())
                                    ...
                                    @foreach($bookings as $booking)
                                        <tr class="border-b last:border-0 hover:bg-gray-50 align-top">
                                            <td class="px-4 py-3">
                                                ...
                                                @if($booking->join_start && $booking->join_end)
                                                    <div class="text-xs text-gray-500 mt-1">
                                                        Student window:
                                                        {{ $booking->join_start->format('h:i A') }} - {{ $booking->join_end->format('h:i A') }}
                                                        ({{ (int) ceil($booking->join_duration_seconds / 60) }} mins)
                                                    </div>
                                                    <div class="text-xs text-gray-500">
                                                        Position {{ $booking->join_position }}/{{ $booking->join_total }}
                                                    </div>
                                                @endif
                                            </td>
                                            <td class="px-4 py-3">

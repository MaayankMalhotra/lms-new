    ...
    <div class="max-w-4xl mx-auto bg-white rounded-xl shadow-lg p-8">
        <div class="mb-6 border-b pb-4">
            <h2 class="text-2xl font-bold text-gray-800 flex items-center">
                <i class="fas fa-edit text-blue-500 mr-2"></i>Edit Webinar
            </h2>
            <p class="text-gray-500 mt-1">Modify the webinar details and update below.</p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.webinar.update', $webinar->id) }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="url" name="image_url" value="{{ old('image_url', $webinar->image_url) }}"
                ...
                <div>
                    ...
                    <input type="datetime-local" name="start_time" value="{{ old('start_time', $webinar->start_time->format('Y-m-d\TH:i')) }}"
                        class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-blue-400 focus:outline-none" required>
                </div>

                <div>
                    <label class="block text-gray-700 font-medium mb-2">Registration Deadline</label>
                    <input type="datetime-local" name="registration_deadline" value="{{ old('registration_deadline', $webinar->registration_deadline->format('Y-m-d\TH:i')) }}"
                        class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-blue-400 focus:outline-none" required>
                </div>

                <div>
                    <label class="block text-gray-700 font-medium mb-2">Entry Type</label>
                    <input type="text" name="entry_type" value="{{ old('entry_type', $webinar->entry_type) }}"
                        class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-blue-400 focus:outline-none">
                </div>

                <div>
                    <label class="block text-gray-700 font-medium mb-2">Google Meet Link</label>
                    <input type="url" name="meeting_link" value="{{ old('meeting_link', $webinar->meeting_link) }}"
                ...
                <div>
                    ...
                    <input type="number" name="participants_count" value="{{ old('participants_count', $webinar->participants_count) }}"
                        class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-blue-400 focus:outline-none">
                </div>

                <div class="md:col-span-2">
                    <label class="block text-gray-700 font-medium mb-2">Tags</label>
                    <input type="text" name="tags" value="{{ old('tags', $webinar->tags) }}"
                        class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-blue-400 focus:outline-none">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="4"
                        class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-blue-400 focus:outline-none">{{ old('description', $webinar->description) }}</textarea>
                </div>
            </div>
